...
| <a name="input_healthcheck_httpcodes_to_match"></a> [healthcheck\_httpcodes\_to\_match](#input\_healthcheck\_httpcodes\_to\_match) | A comma-separated list of HTTP code(s) that indicate that an instance is healthy. Note that this option is only applicable to environments with a network or application load balancer | `list(string)` | `[]` | no |
